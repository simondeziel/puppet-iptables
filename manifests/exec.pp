...
class firewall::exec inherits firewall {
  ...
  if $use_snippets {
    ...
    exec { 'iptables-aggregate':
      ...
      umask   => '0077',
      # only if missing aggregated file or if aggregated file is older than snippet dir
      onlyif  => "[ ! -e \"${iptables_aggregated_file}\" ] || [ \"${iptables_aggregated_file}\" -ot \"${snippet_dir}\" ]",
    }
